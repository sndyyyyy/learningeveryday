                ...
                <table class="w-full text-left border-collapse min-w-[650px]"> 
                    <thead>
                        <tr class="border-b border-gray-200 text-gray-400 text-xs uppercase bg-gray-50">
                            ...
                            <th class="py-3 px-3 text-center">Password</th>
                    ...
                    <tbody class="text-gray-600 text-xs md:text-sm divide-y divide-gray-100">
                        @forelse($peserta as $index => $p)
                            <tr class="hover:bg-gray-50/70 transition">
                                ...
                                <td class="py-3.5 px-3">{{ $p->email }}</td>
                                <td class="py-3.5 px-3 text-center font-mono font-bold text-gray-700 whitespace-nowrap">
                                    {{ $p->raw_password ?? '***' }}
                                </td>
...
<td class="py-3.5 px-3">
    <div class="flex flex-row items-center justify-center gap-2 flex-nowrap">
        ...
        <form action="{{ route('admin.peserta.reset_password', $p->id) }}" method="POST" onsubmit="return confirm('Yakin ingin mereset sandi peserta ini ke default (password123)?')">
            @csrf
            <button type="submit" class="bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold px-3 py-1.5 rounded-md transition whitespace-nowrap cursor-pointer">
                Reset
            </button>
        </form>
        
        <form action="{{ route('admin.peserta.destroy', $p->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus?')">
            @csrf @method('DELETE')
            <button type="submit" class="text-red-500 hover:text-white hover:bg-red-500 font-bold text-xs bg-red-50 px-3 py-1.5 rounded-md transition cursor-pointer whitespace-nowrap">
                Hapus
            </button>
        </form>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="py-4 text-center text-gray-400">Belum ada peserta.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
