...
<head>
    ...
    <title>Detail Hasil - {{ $quiz->title }}</title>
...
<body class="bg-gray-100 font-sans pb-16">
    ...
    <div class="bg-white border-t border-gray-100 py-4 px-4 md:px-8 shadow-xs sticky top-16 z-30">
        <div class="max-w-4xl mx-auto flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <a href="{{ route('peserta.riwayat.index') }}" class="text-xs text-gray-500 hover:text-indigo-600 font-semibold transition">&larr; Kembali ke Riwayat</a>
                <h2 class="text-base md:text-lg font-bold text-gray-800 mt-1">Review Jawaban: {{ $quiz->title }}</h2>
            </div>
            <div class="flex items-center space-x-3 bg-gray-50 p-2 px-4 rounded-xl border border-gray-100">
                <span class="text-xs font-medium text-gray-500">Skor Kamu:</span>
    ...
    <div class="max-w-3xl mx-auto mt-8 px-4 space-y-6">
        ...
        @foreach($questions as $index => $q)
            @php
                // Ambal jawaban yang dipilih peserta untuk soal ID ini dari database JSON
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border {{ $isCorrect ? 'border-emerald-100' : 'border-red-100' }}">
                ...
                @if($q->image)
                    <div class="mb-4">
                        <img src="/storage/{{ $q->image }}" alt="Gambar Soal" class="max-h-48 object-contain rounded-xl border border-gray-100 shadow-2xs">
                    </div>
                @endif
                @if($q->audio)
                    <div class="mb-4">
                        <audio src="/storage/{{ $q->audio }}" controls class="w-full max-w-sm outline-none scale-90 origin-left"></audio>
                    </div>
                @endif
                ...
                <div class="space-y-2 pl-2">
                    @foreach($q->options as $key => $optionText)
                        @php
                            ...
                            // Aturan Pewarnaan Opsi:
                            if ($key === $q->correct_answer) {
                                // Jika ini kunci jawaban asli, selalu warnai hijau
                                $bgClass = 'bg-emerald-50 text-emerald-700 border border-emerald-300 font-semibold';
                            } elseif ($key === $userAnswer && !$isCorrect) {
                                // Jika ini jawaban salah pilihan user, warnai merah mendalam
                                $bgClass = 'bg-red-50 text-red-700 border border-red-200 line-through';
                            }
                        @endphp

                        <div class="p-3.5 rounded-xl text-sm transition flex justify-between items-center {{ $bgClass }}">
                            ...
                            @elseif($key === $userAnswer)
                                <span class="text-red-500 text-xs font-bold font-mono">[Jawaban Kamu]</span>
                            @endif
                        </div>
                    @endforeach
                </div>
                ...
                @if($q->explanation)
                    <div class="mt-4 bg-indigo-50/50 border-l-4 border-indigo-600 p-4 rounded-r-xl text-xs md:text-sm leading-relaxed">
                        <span class="font-bold text-indigo-700 block mb-1">💡 Pembahasan Soal:</span>
